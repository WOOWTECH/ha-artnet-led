...
# Import constants from const.py for config entry support
from .const import (
    DOMAIN,
    CONF_HOST,
    CONF_PORT,
    ...
    CONF_PORT_OVERRIDE,
    CONF_UNIVERSES,
    CONF_OUTPUT_CORRECTION as CONF_OUTPUT_CORRECTION_CONST,
    CONF_SEND_PARTIAL_UNIVERSE as CONF_SEND_PARTIAL_CONST,
    CONF_DEVICES as CONF_DEVICES_CONST,
    CONF_FIXTURE_ID,
    CONF_CHANNEL as CONF_CHANNEL_CONST,
    CONF_NAME as CONF_NAME_CONST,
    CONF_TYPE as CONF_TYPE_CONST,
    CONF_TRANSITION as CONF_TRANSITION_CONST,
    CONF_CHANNEL_SIZE as CONF_CHANNEL_SIZE_CONST,
    CONF_BYTE_ORDER as CONF_BYTE_ORDER_CONST,
    CONF_CHANNEL_SETUP as CONF_CHANNEL_SETUP_CONST,
    CONF_MIN_TEMP as CONF_MIN_TEMP_CONST,
    CONF_MAX_TEMP as CONF_MAX_TEMP_CONST,
...
async def async_setup_entry(
    ...
    """Set up Art-Net LED lights from a config entry."""
    pyartnet.base.CREATE_TASK = hass.async_create_task

    config = entry.data
    log.debug("Setting up lights from config entry: %s", config)

    # Extract connection settings
    host = config.get(CONF_HOST)
    port = config.get(CONF_PORT, ARTNET_DEFAULT_PORT)
    node_type = config.get(CONF_NODE_TYPE, NODE_TYPE_ARTNET_DIRECT)
    max_fps = config.get(CONF_MAX_FPS, 25)
    refresh_interval = config.get(CONF_REFRESH_EVERY, 120)

    # Handle overrides
    real_host = config.get(CONF_HOST_OVERRIDE) or host
    real_port = config.get(CONF_PORT_OVERRIDE) or port

    # Create pyartnet node based on node type
    node: pyartnet.base.BaseNode
    ...
    # Store node in hass.data for cleanup
    hass.data[DOMAIN][entry.entry_id]["node"] = node

    entity_registry = async_get(hass)
    device_list = []
    used_unique_ids = []

    # Process universes and devices
    for universe_nr_str, universe_cfg in config.get(CONF_UNIVERSES, {}).items():
        universe_nr = int(universe_nr_str)

        try:
            universe = node.get_universe(universe_nr)
        except UniverseNotFoundError:
            universe: BaseUniverse = node.add_universe(universe_nr)
            output_correction = universe_cfg.get(
                CONF_OUTPUT_CORRECTION_CONST, DEFAULT_OUTPUT_CORRECTION
            )
            universe.output_correction = AVAILABLE_CORRECTIONS.get(output_correction)

        send_partial_universe = universe_cfg.get(CONF_SEND_PARTIAL_CONST, True)
        if not send_partial_universe:
            universe._resize_universe(512)

        for device_cfg in universe_cfg.get(CONF_DEVICES_CONST, []):
            device = dict(device_cfg)

            # Get device properties
            fixture_id = device.get(CONF_FIXTURE_ID, "")
            channel = device.get(CONF_CHANNEL_CONST)
            name = device.get(CONF_NAME_CONST, f"DMX Light {channel}")
            device_type = device.get(CONF_TYPE_CONST, "dimmer")

            # Generate unique_id using fixture_id or fallback
            if fixture_id:
                unique_id = f"{DOMAIN}:{entry.entry_id}:{fixture_id}"
            else:
                unique_id = f"{DOMAIN}:{host}/{universe_nr}/{channel}"
            ...
            entity_id = f"light.{name.replace(' ', '_').lower()}"
            entity = entity_registry.async_get(entity_id)
            if entity and entity.unique_id and entity.unique_id not in used_unique_ids:
                unique_id = entity.unique_id
            used_unique_ids.append(unique_id)

            # Get class for device type
            cls = __CLASS_TYPE.get(device_type, DmxDimmer)

            # Build device config dict for class constructor
            byte_size = CHANNEL_SIZE[device.get(CONF_CHANNEL_SIZE_CONST, DEFAULT_CHANNEL_SIZE)][0]
            byte_order = device.get(CONF_BYTE_ORDER_CONST, DEFAULT_BYTE_ORDER)
            ...
            device_kwargs = {
                ...
                CONF_BYTE_ORDER: byte_order,
                CONF_CHANNEL_SETUP: device.get(CONF_CHANNEL_SETUP_CONST),
                CONF_DEVICE_MIN_TEMP: device.get(CONF_MIN_TEMP_CONST, DEFAULT_MIN_TEMP),
                CONF_DEVICE_MAX_TEMP: device.get(CONF_MAX_TEMP_CONST, DEFAULT_MAX_TEMP),
            }

            # Create device instance
            d = cls(**device_kwargs)
            d.set_type(device_type)
            ...
            # Set output correction
            device_output_correction = device.get(CONF_OUTPUT_CORRECTION_CONST)
            if device_output_correction:
                d.channel.output_correction = AVAILABLE_CORRECTIONS.get(
                    device_output_correction
                )

            # Store fixture_id for entity matching
            d._fixture_id = fixture_id
            d._config_entry_id = entry.entry_id

            device_list.append(d)

    # Store entities for tracking
    hass.data[DOMAIN][entry.entry_id]["entities"] = device_list

    async_add_entities(device_list)
    log.info("Added %d lights from config entry", len(device_list))
...
async def async_unload_entry(
    ...
    """Unload lights from a config entry."""
    log.debug("Unloading lights for config entry: %s", entry.entry_id)
    # Entity cleanup is handled by Home Assistant automatically
    return True
